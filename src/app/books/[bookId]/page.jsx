    ...
    return (
      <div className="min-h-[80vh] bg-[#070b12] flex flex-col items-center justify-center text-center p-6 text-slate-200">
        <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-3xl p-8 max-w-sm shadow-2xl space-y-4">
          <h2 className="text-2xl font-black text-rose-500 uppercase tracking-tight">Book Not Found</h2>
          <p className="text-sm text-slate-400">The book you are looking for might have been removed or does not exist.</p>
          <Link href="/books" className="btn bg-[#8b5cf6] hover:bg-[#7c3aed] text-white font-bold text-xs tracking-widest rounded-xl border-none uppercase">
            Back to All Books
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="relative min-h-screen bg-[#070b12] text-slate-100 py-12 px-4 sm:px-6 lg:px-8 overflow-hidden flex items-center justify-center">
      ...
      <div className="container mx-auto max-w-5xl relative z-10 space-y-6">
        ...
        <Link href="/books" className="inline-flex items-center gap-2 text-xs font-bold text-slate-400 hover:text-white transition-colors uppercase tracking-widest group">
          ...
          <div className="lg:col-span-7 space-y-5">
            ...
            <div className="space-y-1">
              ...
              <p className="text-sm font-bold text-purple-400">
                By: <span className="text-slate-300 font-medium">{bookDetail?.author}</span>
              </p>
            </div>

            {/* ডিভাইডার ও ক্যাটাগরি */}
            ...
            <div className="text-sm leading-relaxed text-slate-300 space-y-2">
              ...
              <p className="font-medium text-slate-400 text-justify">
                {bookDetail?.review || "No review available for this masterpiece yet. Dive into the book to experience its spectacular storytelling, deeply woven subplots, and unforgettable characters that keep readers engaged from start to finish."}
              </p>
            </div>

           
            <div className="flex flex-wrap items-center gap-3 pt-2">
              <span className="text-xs font-black text-white uppercase tracking-wider">Tags:</span>
              <div className="flex flex-wrap gap-2">
                {bookDetail?.tags?.map((tag, index) => (
                  <div key={index} className="px-2.5 py-1 bg-purple-500/10 text-[#8b5cf6] font-bold text-[10px] tracking-wider uppercase rounded-md border border-purple-500/10">
                    #{tag}
                  </div>
                ))}
              </div>
            ...
            <div className="border-t border-white/5 pt-5 space-y-3 text-xs sm:text-sm font-medium">
              <div className="grid grid-cols-2 max-w-sm">
                <span className="text-slate-500 font-bold uppercase tracking-wider">Number of Pages:</span>
                <span className="text-slate-200 font-semibold">{bookDetail?.totalPages || "N/A"}</span>
              </div>
              <div className="grid grid-cols-2 max-w-sm">
                <span className="text-slate-500 font-bold uppercase tracking-wider">Publisher:</span>
                <span className="text-slate-200 font-semibold">{bookDetail?.publisher || "N/A"}</span>
              </div>
              <div className="grid grid-cols-2 max-w-sm">
                <span className="text-slate-500 font-bold uppercase tracking-wider">Year of Publishing:</span>
                <span className="text-slate-200 font-semibold">{bookDetail?.yearOfPublishing || "N/A"}</span>
              </div>
              <div className="grid grid-cols-2 max-w-sm">
                <span className="text-slate-500 font-bold uppercase tracking-wider">Rating:</span>
            ...
            <div className="pt-6 flex flex-wrap gap-4">
              <button className="px-5 py-3 bg-white/5 hover:bg-white/10 border border-white/10 text-slate-300 hover:text-white font-extrabold text-xs tracking-widest rounded-xl transition-all uppercase active:scale-[0.95]">
                Read Book
              </button>
